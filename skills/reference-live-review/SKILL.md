...
10. If the workflow should end in a reusable machine-readable verdict, run:
   ```bash
   PYTHONPATH="${CLAUDE_PLUGIN_ROOT}" python3 -m webview_screenshort.cli.qa_verdict <live-replay.json> --output-format json
   ```
11. If the workflow should end in a threshold-aware gate result directly, prefer `/reference-live-gate` instead of chaining verdict + gate manually.
