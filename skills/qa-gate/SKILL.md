...
argument-hint: <compare-session.json|comparison.json|live-replay.json|verdict.json> [--policy-preset <name>] [--policy-file <policy.json>] [--fail-on-invalid true|false] [--require-device desktop] [--require-device tablet] [--require-device mobile] [--max-diff-pixels <n>] [--max-diff-ratio <n>] [--output-format json|text]
